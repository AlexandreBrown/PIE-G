...
def _load_places(data_dir, batch_size=256, image_size=84, num_workers=8, use_val=False):
    global places_dataloader, places_iter
    partition = "val" if use_val else "train"
    print(f"Loading {partition} partition of places365_standard...")
    assert os.path.exists(
        data_dir
    ), f"Can't augment observation with Places365 dataset since {data_dir} does not exist!"
...
def random_overlay(x, data_dir, device, dataset="places365_standard"):
    ...
    if dataset == "places365_standard":
        if places_dataloader is None:
            _load_places(data_dir=data_dir, batch_size=x.size(0), image_size=x.size(-1))
        imgs = _get_places_batch(batch_size=x.size(0), device=device).repeat(
            1, x.size(1) // 3, 1, 1
        )
    else:
